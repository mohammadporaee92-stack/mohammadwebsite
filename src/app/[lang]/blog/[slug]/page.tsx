...
export async function generateMetadata({ params }: { params: Promise<{ lang: string; slug: string }> }): Promise<Metadata> {
  ...
  const url = absoluteUrl(`/${l}/blog/${slug}`);
  return {
    title, description: desc,
    alternates: { canonical: url, languages: { fa: absoluteUrl(`/fa/blog/${slug}`), en: absoluteUrl(`/en/blog/${slug}`) } },
...
export default async function BlogPost({ params }: { params: Promise<{ lang: string; slug: string }> }) {
  ...
  if (!article || article.kind !== "article" || article.status !== "published") notFound();
  ...
  if (user) {
    ...
    await recordHistory({
      ...
      title: pick(article, "title", lang), url: `/${lang}/blog/${slug}`,
    });
  }

  return <ArticleDetail lang={lang} article={article} bookmarked={bookmarked} base="blog" />;
}
